...
scene number factor = f factor (g number primes assortedColors)
...
f :: Double -> [(Integer, Color)] -> Picture
f factor pairs = dilated factor (pictures [ translated (fromIntegral x) 0 (colored c (solidRectangle 5 5)) | (x,c) <- pairs ])

g :: Int -> [a] -> [b] -> [(a, b)]
g number xs ys = reverse (take number (zip xs ys))
